...
async function processLanguageFiles(
  ...
  function updateNestedStructure(source, target, langCode, originalLangJson) {
    const stats = { updatedCount: 0, unchangedCount: 0, missingTranslations: [] };
    const sourceLanguage = langCodes[0];
    ...
    if (stats.missingTranslations.length > 0) {
      console.warn(chalk.yellow(t("missingTranslationsWarning", { 
        lang: langCode, 
        count: stats.missingTranslations.length 
      })));
      stats.missingTranslations.forEach(item => {
        console.warn(chalk.yellow(t("missingTranslationItem", { 
          path: item.path, 
