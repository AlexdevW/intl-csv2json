...
async function processMultiLanguage(options) {
  ...
  try {
    ...
    // 抽取 CSV 解析逻辑到单独的函数
    const { zhToLangMap, translationContexts, recordCount, skippedRecords } = parseCSVContent(
      csvContent,
      langCodes,
      trim
    );
...
function parseCSVContent(content, langCodes, trim) {
  ...
  const records = parse(content, {
    ...
    on_skip: (error, rawRecord, context) => {
      // 通过行号获取原始行内容
      const lineNumber = context?.lines || error.lines;
      if (lineNumber && lines[lineNumber]) {
        const rawLine = lines[lineNumber];
        const firstColumn = rawLine.split(',')[0]?.replace(/"/g, '').trim();
  ...
  for (let i = 1; i < records.length; i++) {
    ...
    for (let j = 0; j < Math.min(langCodes.length, values.length); j++) {
      ...
      if (translation && translation.trim() !== '') {
        ...
        langMap.get(zhText).push(translation.replace(/\r\n/g, '\n')); // 翻译也标准化
      ...
      for (const key in sourceNode) {
        ...
        if (typeof sourceValue === "object" && sourceValue !== null) {
          ...
          if (translationMap?.has(sourceValue)) {
            ...
            // 只在找到翻译时检查是否有解析错误
            const skippedRecord = skippedRecords?.find(record => record.zhText === sourceValue);
            if (skippedRecord) {
              console.error(`⚠️  模板文本CSV解析错误: ${skippedRecord.error} (文本: "${sourceValue}")`);
            }
          } else {
