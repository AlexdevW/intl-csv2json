  ...
  for (let i = 0; i < langCodes.length; i++) {
    ...
    if (hasChanges) {
      try {
        ...
        console.error(
          chalk.red(t("writeJsonError", { lang: langCode, message: writeError.message }))
        );
      }
    } else {
...
function logProcessingResults(stats) {
  ...
  console.log(chalk.green(t("processedLanguages", { langs: stats.processedLangs.join(", ") })));
  console.log(
    chalk.green(
      t("totalUpdates", { 
  ...
  console.log(chalk.green(t("filesUpdated", { count: stats.totalStats.languages })));
}
